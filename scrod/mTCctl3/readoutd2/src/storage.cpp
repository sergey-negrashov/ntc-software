...
std::vector<int> Storage::getRunAndExperimentNumbers()
{
   ...
   runNumberFile.open("/usr/local/etc/mtc/experimentAndRunNumbers.txt");              //file, in this folder, where this info
   ...
   if(! runNumberFile)
   {
	...
	if(runNumberFile)
	{
		...
		runFile.open ("/usr/local/etc/mtc/experimentAndRunNumbers.txt",std::fstream::trunc);	//file stored in same place as settings.set
...
std::string makeDate(void)
{
    ...
    dateString[0] = '\0';
    now = time(NULL);
    strftime(dateString, 25, "%Y_%m_%d", localtime(&now));			//changed to use abbreviated month name for clarity when reading filename
    return std::string(dateString);									//day left first for now for easier sorting of folders
}
            ...
            if(packet[2] == EVENT_HEADER_WORD)
            {
                ...
                if(eventNumber > lastEvent)
                {
                    ...
                    if(timeFile >0)
                    {
                        ...
                        string eventText = boost::lexical_cast<std::string>(eventNumber) + " : "  + boost::lexical_cast<std::string>(timeStamp) + "\n";
